...
class Physics
{
    ...
    float ANTI_CLIP_STRENGTH = 0.5f;
    float WORLD_GRAVITY = -0.02f;
            ...
            for (var obj2p : PHYSICS_OBJECTS)
            {
                ...
                if (obj1.HIT_BOX_SQUARE.size() != 0 && obj2.HIT_BOX_CIRCLE.size() != 0)
                {
                    ...
                    */
                    ...
                    //handle collision
                    var elasticity = (obj1.ELASTICITY * obj2.ELASTICITY);
                    if (obj1.TYPE != Type.Static && obj2.TYPE != Type.Static)
                    {
                        if(dir.x != 0)
                            obj2.VELOCITY.x = dir.multiply(-Math.abs(obj2.VELOCITY.x)).x * elasticity;
                        if(dir.y != 0)
                            obj2.VELOCITY.y = dir.multiply(-Math.abs(obj2.VELOCITY.y)).y * elasticity;

                        if(dir.x != 0)
                            obj1.VELOCITY.x = dir.multiply(Math.abs(obj2.VELOCITY.x)).x * elasticity;
                        if(dir.y != 0)
                            obj1.VELOCITY.y = dir.multiply(Math.abs(obj2.VELOCITY.y)).y * elasticity;
                    }
                    if (obj1.TYPE != Type.Static && obj2.TYPE == Type.Static)
                    {
                        var mag1 = obj1.VELOCITY.mag();
                    ...
                    //anti clip
                    float antiClipX = Math.abs(overlap.x) * ANTI_CLIP_STRENGTH;
                    float antiClipY = Math.abs(overlap.y) * ANTI_CLIP_STRENGTH;
                    if(obj1.TYPE != Type.Static)
                    {
                        obj1.POS.x += dir.x * antiClipX;
                        obj1.POS.y += dir.y * antiClipY;
                    }
                    if(obj2.TYPE != Type.Static)
                    {
                        obj2.POS.x -= dir.x * antiClipX;
                        obj2.POS.y -= dir.y * antiClipY;
                    }
                    obj1.hitNormal = dir;
                    obj2.hitNormal = dir.multiply(-1.0f);
                }
                //endregion
                ...
                {
                    Square c1 = obj1.HIT_BOX_SQUARE.get(0); //collider 1, square
                    Square c2 = obj2.HIT_BOX_SQUARE.get(0); //collider 2, circle
                    if(obj1.TYPE == Type.Static && obj2.TYPE == Type.Static)
                        continue;

                    Vector3 c1Pos = obj1.POS.add(new Vector3(c1.ORIGIN.x,c1.ORIGIN.y,0.0f));
                    Vector3 c2Pos = obj2.POS.add(new Vector3(c2.ORIGIN.x,c2.ORIGIN.y,0.0f));
                    Vector2 relativePos = new Vector2(Math.abs(c1Pos.x - c2Pos.x), Math.abs(c1Pos.y - c2Pos.y));

                    if(Math.abs(relativePos.x) > c1.widthHeight.x + c2.widthHeight.x)
                        continue;
                    if(Math.abs(relativePos.y) > c1.widthHeight.y + c2.widthHeight.y)
                        continue;
                    //find the normal
                    Vector2 overlap = new Vector2((c2.widthHeight.x + c1.widthHeight.x) - relativePos.x, (c2.widthHeight.y + c1.widthHeight.y) - relativePos.y);
                    Vector2 dir;

                    Vector2 signedRelPos = new Vector2(c1Pos.x - c2Pos.x, c1Pos.y - c2Pos.y);
                    var elasticity = (obj1.ELASTICITY * obj2.ELASTICITY);

                    if(overlap.x < overlap.y)//x axis
                    {
                        if(signedRelPos.x > 0)
                            dir = new Vector2(1.0f,0.0f);
                        else
...
class Object
{
    ...
    Vector2 VELOCITY;
    float ANGULAR_VEL;
    Vector2 hitNormal = new Vector2(0f,0f);
